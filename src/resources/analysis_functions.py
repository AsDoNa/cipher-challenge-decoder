...
def index_of_coincidence(text:str, alphabet:str=ALPHABET, n:int=1, include_spaces:bool=False, case_insensitive:bool=True, normalised:bool=True):
    '''Determines the index of coincidence of a text.'''
    if include_spaces:
        alphabet += " "
    ...
    text = strip_to_alphabet(text, alphabet)
    text_length = len(text)
    
    # CALCULATE N-GRAMS
    ngrams = calculate_nonoverlapping_ngrams(text=text,n=n,alphabet=alphabet)
    ioc = 0

    for ngram in ngrams.keys():
        ioc += ((ngrams[ngram]*(ngrams[ngram]-1))/(text_length*(text_length-1)))

    normalised_ioc = (len(alphabet)**n)*ioc

    return normalised_ioc if normalised else ioc

class IndexesOfCoincidence():
    def __init__(self):
        self.text_descs = []
        self.mono_iocs = []
        self.bi_iocs = []
    ...
    def add(self, text:str, text_desc:str):
        self.text_descs.append(text_desc)
        self.mono_iocs.append(float(index_of_coincidence(text,n=1, alphabet=ALPHABET, include_spaces=False)))
        self.bi_iocs.append(float(index_of_coincidence(text,n=2, alphabet=ALPHABET, include_spaces=False)))
        self.tri_iocs.append(float(index_of_coincidence(text,n=3, alphabet=ALPHABET, include_spaces=False)))
        self.quad_iocs.append(float(index_of_coincidence(text,n=4, alphabet=ALPHABET, include_spaces=False)))
        self.quinta_iocs.append(float(index_of_coincidence(text,n=5, alphabet=ALPHABET, include_spaces=False)))

    def load(self, directory:str,file:str,mid_sep:str=MID_SEP,end_sep:str=END_SEP):
        with open(os.path.join(directory,file)) as f:
            fileSepped = f.read().split(end_sep)
            fileSepped = [i for i in fileSepped if len(i) > 0]
            text_descs,mono,bi,tri,quad,quinta = fileSepped
        self.text_descs = text_descs.split(mid_sep)
        self.mono_iocs = mono.split(mid_sep)
        self.mono_iocs = [float(i) for i in self.mono_iocs]
        self.bi_iocs = bi.split(mid_sep)
        self.bi_iocs = [float(i) for i in self.bi_iocs]
        self.tri_iocs = tri.split(mid_sep)
        self.tri_iocs = [float(i) for i in self.tri_iocs]
        self.quad_iocs = quad.split(mid_sep)
        self.quad_iocs = [float(i) for i in self.quad_iocs]
        self.quinta_iocs = quinta.split(mid_sep)
        self.quinta_iocs = [float(i) for i in self.quinta_iocs]

    def save(self, directory:str, file:str, mid_sep:str=MID_SEP,end_sep:str=END_SEP):
        with open(os.path.join(directory,file), "w") as f:
            text_descs_str = mid_sep.join([text_desc for text_desc in self.text_descs])
            f.write(f"{text_descs_str}{end_sep}")
            mono_iocs_str = mid_sep.join(["{:.2f}".format(float(mono_ioc)) for mono_ioc in self.mono_iocs])
            f.write(f"{mono_iocs_str}{end_sep}")
            bi_iocs_str = mid_sep.join(["{:.2f}".format(float(bi_ioc)) for bi_ioc in self.bi_iocs])
            f.write(f"{bi_iocs_str}{end_sep}")
            tri_iocs_str = mid_sep.join(["{:.2f}".format(float(tri_ioc)) for tri_ioc in self.tri_iocs])
            f.write(f"{mono_iocs_str}{end_sep}")
            quad_iocs_str = mid_sep.join(["{:.2f}".format(float(quad_ioc)) for quad_ioc in self.quad_iocs])
            f.write(f"{quad_iocs_str}{end_sep}")
            quinta_iocs_str = mid_sep.join(["{:.2f}".format(float(quinta_ioc)) for quinta_ioc in self.quinta_iocs])
            f.write(f"{quinta_iocs_str}{end_sep}")

    def return_averages(self):
        return [mean(self.mono_iocs),mean(self.bi_iocs),mean(self.tri_iocs),mean(self.quad_iocs),mean(self.quinta_iocs)]

def compare_to_english(text:str,English_IoCs:IndexesOfCoincidence):
    text_mono_ioc = index_of_coincidence(text,n=1, alphabet=ALPHABET, include_spaces=False)
    text_bi_ioc = index_of_coincidence(text,n=2, alphabet=ALPHABET, include_spaces=False)
    text_tri_ioc = index_of_coincidence(text,n=3, alphabet=ALPHABET, include_spaces=False)
    text_quad_ioc = index_of_coincidence(text,n=4, alphabet=ALPHABET, include_spaces=False)
    text_quinta_ioc = index_of_coincidence(text,n=5, alphabet=ALPHABET, include_spaces=False)

    print("SIMILARITY TO INDEXES OF COINCIDENCE")
    print(f"OVERALL: {chi_squared([text_mono_ioc,text_bi_ioc,text_tri_ioc,text_quad_ioc,text_quinta_ioc],English_IoCs.return_averages())}")
    print(f"MONOGRAMS: {chi_squared([text_mono_ioc],[English_IoCs.return_averages()[0]])}")
    print(f"BIGRAMS: {chi_squared([text_bi_ioc],[English_IoCs.return_averages()[1]])}")
    print(f"TRIGRAMS: {chi_squared([text_tri_ioc],[English_IoCs.return_averages()[2]])}")
    print(f"QUADGRAMS: {chi_squared([text_quad_ioc],[English_IoCs.return_averages()[3]])}")
    print(f"QUINTAGRAMS: {chi_squared([text_quinta_ioc],[English_IoCs.return_averages()[3]])}")
    
def entropy(text:str): # Modify to take alphabet and log alphabet_length?
    '''Calculate the entropy of a piece of text (English 0.88~0.90; Random ~1)'''
    freqs = count_monograms(text,"alphabet",True,False,False,False,"percent")
    entropy = 0

    for monogram in freqs.keys():
        if freqs[monogram] != 0: # TODO FIX THIS
            entropy -= (freqs[monogram]*log(freqs[monogram],26))

    return entropy

if __name__ == '__main__':
    text = '''Book the First--Recalled to Life
...
far, far better rest that I go to than I have ever known.”'''
    # text_desc = "-ProjGut"
    # english = IndexesOfCoincidence()
    # english.load("src/resources","english_iocs.txt")
    # english.add(text,text_desc)
    # english.save("src/resources", "english_iocs.txt")
    # averages = ["{:.2f}".format(float(average)) for average in english.return_averages()]
    # print(" ".join(averages))
    print(index_of_coincidence(text,n=1, alphabet=ALPHABET, include_spaces=False))
    print(index_of_coincidence(text,n=2, alphabet=ALPHABET, include_spaces=False))
    print(index_of_coincidence(text,n=3, alphabet=ALPHABET, include_spaces=False))
    print(index_of_coincidence(text,n=4, alphabet=ALPHABET, include_spaces=False))
    print(index_of_coincidence(text,n=5, alphabet=ALPHABET, include_spaces=False))
    # compare_to_english(text,english)
    # print(entropy(text))
    pass